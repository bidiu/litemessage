...
// EXTERNAL MODULE: external "leveldown"
var external_leveldown_ = __webpack_require__(4);
var external_leveldown_default = /*#__PURE__*/__webpack_require__.n(external_leveldown_);

// EXTERNAL MODULE: external "levelup"
var external_levelup_ = __webpack_require__(5);
var external_levelup_default = /*#__PURE__*/__webpack_require__.n(external_levelup_);
...
var external_ws_ = __webpack_require__(3);
...
// CONCATENATED MODULE: ./src/wss.js
 ...
 */
class wss_WSServer extends external_events_default.a {
  constructor(uuid, nodeType, { port = 1113 } = {}) {
    ...
    this.wss = new external_ws_default.a.Server({ port });
  ...
  createConnection(url) {
    ...
    try {
      ({ host: socketAddress, port: remoteDaemonPort } = new external_url_["URL"](url));
    ...
    let socket = new external_ws_default.a(url, { headers: this.headers, handshakeTimeout: 10000 });
  ...
  socketAbnormal(socket) {
    return !socket.alive && socket.readyState === external_ws_default.a.OPEN;
  }

  socketAlive(socket) {
    return socket.readyState === external_ws_default.a.OPEN;
  }
...
/* harmony default export */ var wss = (wss_WSServer);
...
/* harmony default export */ var src_peer = (peer_Peer);

// EXTERNAL MODULE: external "uuid/v1"
var v1_ = __webpack_require__(9);
...
// CONCATENATED MODULE: ./src/litenode.js
 ...
 * to worry about).
 */
class litenode_LiteNode extends external_events_default.a {
  /**
  ...
  constructor(nodeType, { port, debug = true } = {}) {
    ...
    this.wss = new wss(this.uuid, this.nodeType, { port });
    // when bound to an network interface
  ...
  connectionHandler(socket, incoming, remoteUuid, remoteDaemonPort, remoteNodeType) {
    ...
    let newPeer = new src_peer(remoteUuid, socket, incoming, remoteDaemonPort, remoteNodeType);
...
/* harmony default export */ var litenode = (litenode_LiteNode);
...
const client_nodeTypes = ['full', 'thin'];
const client_nodeType = 'thin';

class client_ThinClient {
  constructor(protocolClass, dbPath, { port = 1113, initPeerUrls = [] } = {}) {
    ...
    this.litenode = new litenode(client_nodeType, { port });

    // load protocol
    this.protocol = new protocolClass(this, client_nodeTypes);
  ...
  static get nodeType() {
    return client_nodeType;
  }

  initDb(dbPath) {
    if (external_fs_default.a.existsSync(dbPath) && external_fs_default.a.statSync(dbPath).isDirectory()) {
      console.log('Using existing LevelDB directory.');
    } else {
      console.log('A new LevelDB directory will be created.');
    }
    this.db = external_levelup_default()(external_leveldown_default()(dbPath));
  }
...
/* harmony default export */ var store = (P2PProtocolStore);

// CONCATENATED MODULE: ./src/p2pprotocol/messages.js
...
// CONCATENATED MODULE: ./src/p2pprotocol/p2protocol.js
...
// look up dns records
const lookup = Object(external_util_["promisify"])(external_dns_default.a.lookup);
  ...
  async connectToLastConnectedPeers() {
    try {
      ...
      let addresses = await Promise.all(
        initUrls.map(url => lookup(new external_url_["URL"](url).hostname, { family: 4 }))
      );

      initUrls = initUrls.map((url, i) => {
        url = new external_url_["URL"](url);
...
/* harmony default export */ var p2protocol = (p2protocol_P2PProtocol);

// CONCATENATED MODULE: ./src/client/protocol.js


/**
 * TODO later might change to extend liteprotocol
 */
const protocol_protocolClass = class extends p2protocol {
  constructor(node, nodeTypes, { minPeerNum = 8 } = {}) {
    super(node, nodeTypes, { minPeerNum });
  }
};

/* harmony default export */ var protocol = (protocol_protocolClass);

// EXTERNAL MODULE: external "child_process"
var external_child_process_ = __webpack_require__(12);
...
var external_bluebird_ = __webpack_require__(7);
var external_bluebird_default = /*#__PURE__*/__webpack_require__.n(external_bluebird_);

// CONCATENATED MODULE: ./src/utils/litecrypto.js
...
external_bluebird_default.a.config({
  // enable warnings
  warnings: true,
...
const sha256 = (content, digest = 'hex') => {
  return external_crypto_default.a.createHash('sha256')
    .update(content)
    .digest(digest);
};
...
const mine = (content, difficulty) => 
  new external_bluebird_default.a((resolve, reject, onCancel) => {
    if (typeof difficulty !== 'number') { reject(new Error('Invalid difficulty.')); }
    console.log(external_path_default.a.join(__dirname, 'mine.js'));
    let cp = Object(external_child_process_["fork"])(external_path_default.a.join(__dirname, 'mine.js'), [content, difficulty]);
...
// CONCATENATED MODULE: ./src/liteprotocol/entities/litemsg.js
...
/* harmony default export */ var entities_litemsg = (createLitemsg);

// CONCATENATED MODULE: ./src/liteprotocol/store.js
const store_prefix = 'lite/';

const store_genKey = key => store_prefix + key;
...
class LiteProtocolStore {
  ...
  async readLitemsg(litemsgId) {
    try {
      let buf = await this.db.get(store_genKey(`litemsg_${litemsgId}`));
      return buf.toString();
  ...
  async readHeadBlock() {
    try {
      let buf = await this.db.get(store_genKey('head_block'));
      return buf.toString();
    } catch (err) {
      if (err.notFound) { return undefined; }
      throw err;
  ...
  async readBlock(blockId) {
    try {
      let buf = await this.db.get(store_genKey(`block_${blockId}`));
      return JSON.parse(buf.toString());
  ...
  async appendBlock(block, batchOps) {
    ...
    let ops = [
      { type: 'put', key: store_genKey(`block_${block.hash}`), value: JSON.stringify(block) },
      { type: 'put', key: store_genKey('head_block'), value: block.hash }
    ];
    for (let litemsg of block.litemsgs) {
      ops.push({ type: 'put', key: store_genKey(`litemsg_${litemsg.hash}`), value: block.hash });
    }
    if (batchOps) {
      ops = [...ops, ...batchOps];
    }
  ...
  async appendBlocksAt(blocks, batchOps) {
    ...
    for (let block of blocks) {
      ...
      ops.push({ type: 'put', key: store_genKey(`block_${block.hash}`), value: JSON.stringify(block) });
      for (let litemsg of block.litemsgs) {
        ops.push({ type: 'put', key: store_genKey(`litemsg_${litemsg.hash}`), value: block.hash });
      }
    }
    ops.push({ type: 'put', key: store_genKey('head_block'), value: headBlock.hash });
...
/* harmony default export */ var liteprotocol_store = (LiteProtocolStore);

// CONCATENATED MODULE: ./src/liteprotocol/entities/block.js
...
/* harmony default export */ var entities_block = (createBlock);

// CONCATENATED MODULE: ./src/liteprotocol/miner.js
...
class miner_Miner {
  ...
  mine(block) {
    ...
    return this.calc(content, bits)
      .then(nonce => 
        entities_block(ver, time, height, prevBlock, merkleRoot, bits, nonce, litemsgs));
  }
...
/* harmony default export */ var miner = (miner_Miner);

// CONCATENATED MODULE: ./src/utils/blockchain.js
 ...
 */
class blockchain_Blockchain extends external_events_default.a {
  constructor(store) {
    super();
    this.store = store;
    this.db = store.db;
...
/* harmony default export */ var utils_blockchain = (blockchain_Blockchain);
...
var external_body_parser_ = __webpack_require__(8);
var external_body_parser_default = /*#__PURE__*/__webpack_require__.n(external_body_parser_);

// CONCATENATED MODULE: ./src/liteprotocol/rest.js
...
function createRestServer(liteProtocol) {
  ...
  return external_http_default.a.createServer(app);
}

/* harmony default export */ var rest = (createRestServer);

// CONCATENATED MODULE: ./src/liteprotocol/messages.js
const messages_messageTypes = Object.freeze({
...
const getBlocks = ({ blockLocators }) => ({
  messageType: messages_messageTypes.getBlocks,
  blockLocators
});
...
const inv = ({ blocks = [], litemsgs = [] }) => ({
  messageType: messages_messageTypes.inv,
  blocks,
  litemsgs
});
...
const getData = ({ blocks = [], litemsgs = [] }) => ({
  messageType: messages_messageTypes.getData,
  blocks,
  litemsgs
});
...
const messages_data = ({ blocks = [], litemsgs = [] }) => ({
  messageType: messages_messageTypes.data,
  blocks,
  litemsgs
});

messages_data.validate = ({ blocks, litemsgs }) => {
  if (!(blocks instanceof Array)) {
    throw new Error('lite/: Invalid blocks.');
  }
  if (!(litemsgs instanceof Array)) {
...
const getPendingMsgs = () => ({
  messageType: messages_messageTypes.getPendingMsgs
});
...
// validators
const messages_messageValidators = Object.freeze({
  [messages_messageTypes.getBlocks]: getBlocks.validate,
  [messages_messageTypes.inv]: inv.validate,
  [messages_messageTypes.getData]: getData.validate,
  [messages_messageTypes.data]: messages_data.validate,
  [messages_messageTypes.getPendingMsgs]: getPendingMsgs.validate
});



// CONCATENATED MODULE: ./src/liteprotocol/liteprotocol.js
...
const liteprotocol_ver = 1;
const liteprotocol_bits = 22;
const blockLimit = 2048;

class liteprotocol_LiteProtocol extends p2protocol {
  static get ver() {
    return liteprotocol_ver;
  }
  ...
  constructor(node, nodeTypes, { minPeerNum = 8 } = {}) {
    ...
    this.liteStore = new liteprotocol_store(node.db);
    // a blockchain manager
    this.blockchain = new utils_blockchain(this.liteStore);
  ...
  init() {
    ...
    this.litenode.on(`message/${messages_messageTypes.getBlocks}`, this.getBlocksHandler);
    this.litenode.on(`message/${messages_messageTypes.inv}`, this.invHandler);
    this.litenode.on(`message/${messages_messageTypes.getData}`, this.getDataHandler);
    this.litenode.on(`message/${messages_messageTypes.data}`, this.dataHandler);
    this.litenode.on(`message/${messages_messageTypes.getPendingMsgs}`, this.getPendingMsgsHandler);
    this.litenode.on('connection', this.connectionHandler);

    // create and run rest server
    rest(this).listen(this.node.port + 1);
  ...
  async getNextBlock() {
    ...
    return entities_block(liteprotocol_ver, time, height, prevBlock, merkleRoot, liteprotocol_bits, undefined, litemsgs);
  }
  ...
  async getBlocksHandler({ messageType, ...payload }, peer) {
    try {
      messages_messageValidators[messageType](payload);
  ...
  async getDataHandler({ messageType, ...payload }, peer) {
    try {
      ...
      if (respBlocks.length || respLitemsgs.length) {
        // send response
        peer.sendJson(
          messages_data({ blocks: respBlocks, litemsgs: respLitemsgs })
        );
      }
...
/* harmony default export */ var liteprotocol = (liteprotocol_LiteProtocol);

// CONCATENATED MODULE: ./src/fullnode.js





const fullnode_nodeTypes = ['full', 'thin'];
const fullnode_nodeType = 'full';
...
class fullnode_FullNode {
  ...
  constructor(protocolClass, dbPath, { port = 1113, initPeerUrls = [] } = {}) {
    ...
    this.initDb(dbPath);
    // create underlying litenode
    this.litenode = new litenode(fullnode_nodeType, { port });
    // load protocol
    this.protocol = new protocolClass(this, fullnode_nodeTypes);
...
/* harmony default export */ var fullnode = (fullnode_FullNode);

// CONCATENATED MODULE: ./index.js
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "ThinClient", function() { return client; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "ThinClientProtocol", function() { return protocol; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "createLitemsg", function() { return entities_litemsg; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "LiteProtocol", function() { return liteprotocol; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "messageTypes", function() { return messages_messageTypes; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "messageValidators", function() { return messages_messageValidators; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "getBlocks", function() { return getBlocks; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "inv", function() { return inv; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "getData", function() { return getData; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "data", function() { return messages_data; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "getPendingMsgs", function() { return getPendingMsgs; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "sha256", function() { return sha256; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "calcMerkleRoot", function() { return calcMerkleRoot; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "verifyMerkleRoot", function() { return verifyMerkleRoot; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "leadingZeroBits", function() { return leadingZeroBits; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "mine", function() { return mine; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "verifyLitemsg", function() { return verifyLitemsg; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "verifyBlock", function() { return verifyBlock; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "verifySubchain", function() { return verifySubchain; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "getCurTimestamp", function() { return getCurTimestamp; });
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "FullNode", function() { return fullnode; });
